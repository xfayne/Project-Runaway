%%Load all subjects files and arrange the data
...
PATH = "xxx" %***Need to add: path to folder

GSR_FILENAME_ST = "_BAT_ST_GS"; %**Need to Notice! files names
GSR_FILENAME_NS = "_BAT_NS_GS";
ECG_FILENAME_ST = "_BAT_ST";
ECG_FILENAME_NS = "_BAT_NS";


%Loop and load 4 files per subject
for i = 1:SUBJECTS
    %Subject's number
    sub_num = strcat("00",i);
    ...
    %(ST)
    file_name = strcat(sub_num, ECG_FILENAME_ST); 
    ecg_st = load(file_name);
    %(NS)
    file_name = strcat(sub_num, ECG_FILENAME_NS); 
    ...
    %(ST)
    file_name = strcat(sub_num, GSR_FILENAME_ST);
    gsr_st = load(file_name);
    %(NS)
    file_name = strcat(sub_num, GSR_FILENAME_NS); 
    ...
    ecg_data{1,1} = ecg_ns.EEG.data;
    ecg_data{2,1} = ecg_st.EEG.data;
